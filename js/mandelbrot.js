...
window.require(["jquery", "libs/modernizr-2.5.3-respond-1.1.0.min", "script", "libs/knockout-2.1.0.debug", "mylibs/mandelbrot-core", "mylibs/color-converter", "mylibs/mandelbrot-colors"], function($, modern, script, ko, MandelbrotCore, ColorConverter, MandelbrotColors) {
  "strict mode";
  ...
  document.addEventListener('progress', (function(event) {
    var data;
    data = JSON.parse(event.data);
    return viewModel.progress(Math.floor(data.value));
  }), false);
  mandelbrot.drawSet();
});
